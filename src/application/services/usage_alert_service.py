...
async def check_usage_alerts(
    ...
    if usage_ratio >= _95_PERCENT and client.alert_95_sent_at is None:
        await _send_alert(client, 95, session_factory, email_adapter)
    elif usage_ratio >= _80_PERCENT and client.alert_80_sent_at is None:
        await _send_alert(client, 80, session_factory, email_adapter)
...
async def _send_alert(
    ...
    try:
        subject = f"Usage Alert: You've reached {threshold}% of your monthly quota"
        body = (
            f"<p>Hi,</p>"
            f"<p>Your Lead Qualification Engine account (<strong>{client.owner_email}</strong>) "
            f"has used <strong>{client.monthly_requests_used}</strong> of "
            f"<strong>{client.monthly_requests_limit}</strong> monthly requests "
            f"({threshold}% threshold reached).</p>"
            f"<p>Current plan: <strong>{client.plan_tier}</strong></p>"
